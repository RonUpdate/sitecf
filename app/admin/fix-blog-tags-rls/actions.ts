...
export async function fixBlogTagsRLS(formData: FormData) {
  try {
    ...
    switch (action) {
      case "disable":
        query = `ALTER TABLE public.blog_tags DISABLE ROW LEVEL SECURITY;`
        break
    ...
    logger.info(`Успешно выполнено действие ${action} для RLS таблицы blog_tags`)

    revalidatePath("/admin/fix-blog-tags-rls")
